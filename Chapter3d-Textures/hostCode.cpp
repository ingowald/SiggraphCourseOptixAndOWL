...
void SampleViewer::buildPipeline()
{
  ...
  OWLVarDecl gtVars[] = {
    ...
    // Delta to previous sample: we added a texture object to the
    // device code, but that means we also need to declare it as a
    // parameter in order to set it.
    { "colorTexture",
      OWL_TEXTURE,
      OWL_OFFSETOF(TrianglesGeomData,colorTexture) },
    // delta from previous sample: declare array of texcoords. should
    // be clear by now how to do this!
    { "texcoords",
      OWL_BUFPTR,
...
void SampleViewer::buildSceneGeometry()
{
  ...
  for (auto miniObject : uniqueObjectsInScene) {
    ...
    for (auto inputMesh : miniObject->meshes) {
      ...
      // ##################################################################
      // Delta to previous sample: let's look at the actual material
      // attached to the geomety, and create/set texture as
      // required. for this sample we only support one of miniScene's
      // material types - this isn't a miniScene tutorial, you'll
      // eventually probably use different importers, anyway.
      // ##################################################################
