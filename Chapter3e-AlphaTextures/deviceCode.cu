...
/* allow us to insert some code pieces to temporarily disable the
   proper handling of alpha, so as to allow the interative viewer to
   turn it on and off */
...
/*! in this sample we evaluate the alpha channel of textures, but
    still only use this to decide if the surface is or isn't opaque at
    the location where the ray has hit it; extension to truly
    semi-transparent surfaces is pretty easy, but none of our models
    has data for that so let's stay with the simple method */
struct ShadowRayPRD {
...
OPTIX_CLOSEST_HIT_PROGRAM(Triangles_CH_RegularRays)()
{
  ...
  // DELTA to previous sample: use actual basecolor from material!
...
/* delta to previous sample: we evaluate alpha of the texture, and if
   the texture is transparent at that location we tell optix to ignore
   that hit. We do this in AH program so optix keeps on searching for
   other hits. Note how this regular-ray AH program is both very
   similar to the shadow ray AH program (they both evaluate the
   texture, and look at the sample's alpha value), but how different
   these two react to that value in the end: for regular rays we
   either tell optix to ignore the hit, or to let it go through to CH;
   for the shadow ray AH pgoram we either tell optix to ignore the
   hit, or mark as occluded and kill the ray. */
OPTIX_ANY_HIT_PROGRAM(Triangles_AH_RegularRays)()
{
#if FOR_ILLUSTRATION_ONLY
  // FOR ILLUSTRATION ONLY: DISABLE THIS TO SHOW EFFECT OF TURNING IT
  // ON AND OFF DURING THE INTERACTIVE SESSION
  if (!optixLaunchParams.doAlphaRegularRays) return;
#endif
  auto self = owl::getProgramData<TrianglesGeomData>();
  if (!self.colorTexture)
...
OPTIX_ANY_HIT_PROGRAM(Triangles_AH_ShadowRays)()
{
  ...
  // FOR ILLUSTRATION ONLY: DISABLE THIS TO SHOW EFFECT OF TURNING IT
  // ON AND OFF DURING THE INTERACTIVE SESSION
  if (!optixLaunchParams.doAlphaShadowRays) {
    // this is exaclty what would have happend in the previous sample,
    // where we didn't properly handle texture transparency.
    prd.isOccluded = true; optixTerminateRay();
  }
#endif
  auto self = owl::getProgramData<TrianglesGeomData>();
  if (!self.colorTexture) {
